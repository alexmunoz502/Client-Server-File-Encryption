...
/* 
* Decryption Server
* Receives encrypted files and keys from decryption clients
* and sends back the plaintext files, can handle up to 5
    ...
    while(RUNNING)
    {
        ...
        if (pid == 0)
        {
            ...
            if (strcmp(buffer, "dec_client") != 0) 
            {
                ...
                perror("SERVER: Unable to Authenticate Client, Not of Type 'dec_client'\n");
                _exit(EXIT_FAILURE);
            } 
            else
            {
                // Valid Client, send back server authentication code
                char authentication_code[] = "dec_server";
                write(connectionSocket, authentication_code, sizeof(authentication_code));
            }

            // Read Size of cyphertext
            uint32_t temp, cyphertext_length;
            read(connectionSocket, &temp, sizeof(temp));
            cyphertext_length = ntohl(temp);
            ...
            // Read the client's message from the socket (cyphertext)
            memset(buffer, '\0', sizeof(buffer));
            socket_file_recv(connectionSocket, buffer, cyphertext_length);
            cyphertext = calloc(strlen(buffer)+1, sizeof(char));  
            ...
            int num_to_write = cyphertext_length;
